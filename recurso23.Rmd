...
x=1:6
fx=rep(1,6)/6
plot(x,fx,pch=19, ylim=c(0,0.18), las=1, xlab="puntos", ylab="f(x)", col=c2)
lines(x, fx, type="h", col=c1, lty=3)
```
:::
::::
...
fx=rep(1,6)/6
Fx=cumsum(fx)
plot(x,Fx, las=1, ylab="F(x)", xlab="puntos",pch=19, ylim=c(0,1), col=c2)
lines(x, Fx, type="s", col=c1)
grid()
```
:::
::: {style="flex: 1;"}
...
kable(tabla, col.names = c("x", "f(x)", "F(x)"), align = 'c', caption = "Tabla de Frecuencias y Distribución Acumulada")
...
$$f_{_{X}}(x) = \left \{
				\begin{matrix}
				\displaystyle\binom{25}{x}(0.7)^{x}(0.3)^{25-x} & \mbox{si }	x=0,1,2,\ldots,25\\
				& \\
				0  & \mbox{en otro caso }
				\end{matrix}\right.
				$$
...
x=0:20
fx=choose(20,x)*0.70^x*0.30^(20-x)
plot(x,fx, las=1, ylab="f(x)", xlab="número personas que asisten al restaurante", pch=19, col=c1)
...
fx=choose(20,x)*0.70^x*0.30^(20-x)
Fx=cumsum(fx)
plot(x,Fx, las=1, ylab="F(x)", xlab="número personas que asisten al restaurante",pch=19, ylim=c(0,1), col=c1)
